...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      -- fzf-lua is a Lua-powered fuzzy finder with a familiar set of pickers for
      -- files, LSP symbols, diagnostics, and more. It relies on `fzf` for the
      -- interface and ships with sensible defaults, but you can tune the layout,
      -- bindings, and preview windows as needed.
      ...
      -- Project-wide search with live updates
      map('<leader>s/', fzf.live_grep, '[S]earch [/] in project')
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          map('gW', function()
            if not pcall(fzf.lsp_live_workspace_symbols) then
          ...
          map('grt', function()
            if not pcall(fzf.lsp_typedefs) then
              fzf.lsp_definitions()
            end
  ...
  -- you can continue same window with `<space>sr` which resumes the last fzf-lua picker
}, {
  ui = {
